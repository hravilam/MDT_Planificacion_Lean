...
Para la evaluación del modelo, se estructuró el dataset \texttt{MDT_FIBRA.csv}, el cual consolida las transacciones históricas permitiendo la validación \textit{out-of-sample} (fuera de muestra), fundamental para la simulación del Punto de Reorden (ROP).
...
\begin{itemize}
    \item \textbf{Enlace al repositorio:} \url{https://github.com/tu_usuario/MDT_Planificacion_Lean}
    \item \textbf{Entorno de ejecución principal:} Google Colab / GitHub Codespaces.
\end{itemize}

%%ANEXO B
\section{Análisis de Estabilización del Flujo de Valor}\label{anex:B}

Como soporte analítico complementario bajo principios \textit{Lean Manufacturing}, las proyecciones multietiqueta del modelo óptimo se acoplaron al cálculo dinámico del Punto de Reorden ($ROP$). Este análisis secundario permitió mapear los tiempos de tránsito del proveedor internacional (\textit{Lead Time} de 90 días) frente a los ritmos de consumo real de la planta, asegurando que la reducción del desperdicio por sobreinventario identificada en la sección de resultados mantenga un nivel de servicio operativo superior al $95\%$ en el suministro de las familias de fibra críticas Clase AAA.
